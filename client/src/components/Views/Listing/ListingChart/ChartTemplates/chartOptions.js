...
export class YearListingChart extends ChartOptions {
  constructor(year, items) {
    ...
    this.axisX = {
      title: `Average Listed per day: ${getAverage(
      ...
      const dataPoints = filteredItems
        .reduce((dataPoints, item) => {
          ...
          if (itemFoundIndex !== -1) {
            ...
            dataPoints.push({ x: dateListed, y: 1, m: profit })
            return dataPoints
          }
        }, [])
        .map((j) => ({ x: new Date(j.x), y: j.y, m: j.m }))
    ...
    function getYearDataPointsByWeek(items) {
      ...
      const dataPoints = filteredItems
        .reduce((dataPoints, item) => {
          ...
          const month = new Date(dateListed).toLocaleString("default", {
            month: "short",
          })
          ...
          if (itemFoundIndex !== -1) {
            ...
            dataPoints.push({ label: `${week} ${month}`, y: 1, m: profit })
            return dataPoints
          }
        }, [])
        .map((j) => ({ label: `W${j.label}`, y: j.y, m: j.m }))
...
export class YearListingChartByMonth extends YearListingChart {
  constructor(year, items) {
    ...
    this.axisX = {
      title: `Average Listing per month: ${getAverage(
      ...
      const dataPoints = filteredItems
        .reduce((dataPoints, item) => {
          ...
          if (itemFoundIndex !== -1) {
            ...
            dataPoints.push({ label: month, y: 1, m: profit })
            return dataPoints
          }
        }, [])
        .map((j) => ({ label: j.label, y: j.y, m: j.m }))
