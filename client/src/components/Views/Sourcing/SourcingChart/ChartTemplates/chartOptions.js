...
export class YearSourcingChart extends ChartOptions {
    constructor(year, items) {
        ...
        this.axisX = {
            title: `Average Sourcing per day: ${getAverage(fillInMissingDays(this.data[0].dataPoints))}`,
            interval: 7,
            xValueType: "date",
        ...
        function getYearDataPoints(items) {
            ...
            const dataPoints = filteredItems.reduce((dataPoints, item) => {
                ...
                const itemFoundIndex = dataPoints.findIndex(x => x.x === datePurchased);
                if (itemFoundIndex !== -1) {
                    dataPoints[itemFoundIndex] = { x: datePurchased, y: dataPoints[itemFoundIndex].y + 1 };
                    return dataPoints;
                } else {
                    dataPoints.push({ x: datePurchased, y: 1 });
                    return dataPoints;
                }
...
export class YearSourcingChartByWeek extends YearSourcingChart {
    constructor(year, items) {
        ...
        this.axisX = {
            title: `Average Sourcing per week: ${getAverage(this.data[0].dataPoints)}`,
            interval: 1
        }
        ...
        function getYearDataPointsByWeek(items) {
            ...
            const dataPoints = filteredItems.reduce((dataPoints, item) => {
                ...
                const month = new Date(datePurchased).toLocaleString('default', { month: 'short' });
...
export class YearSourcingChartByMonth extends YearSourcingChart {
    constructor(year, items) {
        ...
        this.axisX = {
            title: `Average Sourcing per month: ${getAverage(this.data[0].dataPoints)}`,
            interval: 1
        }
        this.axisY = {
            includeZero: true,
        ...
        function getYearDataPointsByMonth(items) {
            ...
            const dataPoints = filteredItems.reduce((dataPoints, item) => {
                const datePurchased = standardDate(item.datePurchased);
                const month = new Date(datePurchased).toLocaleString('default', { month: 'long' });
