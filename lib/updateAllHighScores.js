  ...
  try {
    ...
    for (const category of Object.keys(newScores)) {
      ...
      for (const statType of Object.keys(newScores[category])) {
        ...
        if (isNewHighScore(milestone, category, statType, newStat)) {
          milestone.scores[category][statType] = newStat;
          createAndSaveNotification(userId, category, statType, newStat);
          updated = true;
        }
      }
    }
...
function isNewHighScore(milestone, category, statType, newScore) {
  try {
    if (!milestone || !milestone.scores || !milestone.scores[category]) {
      // Handle the case where milestone or its scores property is missing
      return true; // Treat it as not a new high score to avoid the error
    }

    const currentScore = milestone.scores[category][statType];
    
    // Check if the current score is greater than the new score
    if (currentScore[statType] > newScore[statType]) {
      return false; // The current score is higher, so it's not a new high score
    }

    // Handle the case where comments indicate to treat it as a new high score
    const comments = currentScore.comments;
    if (comments) {
      if (comments.listed || comments.sold || comments.pulled || comments.sales || comments.spent) {
        return true; // Treat it as a new high score based on comments
      }
    }

    // If none of the conditions above are met, it's not a new high score
    return false;
  } catch (error) {
