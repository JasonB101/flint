...
const relistItem = async (itemId, user, settings) => {
  ...
  let [newListedPrice, expectedProfit] = priceReduction(item, settings, averageShippingCost,
    ebayFeePercent)
  ...
  const listingDetails = {
    ...
    lastPriceReduction: newListedPrice !== item.listedPrice
  ...
  //NEXT I NEED TO CREATE AN INVENTORY CHANGES OBJECT TO PASS TO THE INVENTORY ITEM CHANGE FUNCTION
  const inventoryChanges = {
    listedPrice: newListedPrice,
    dateReListed: listingDetails.dateReListed,
    expectedProfit: listingDetails.expectedProfit,
    ebayId: ebayId,
    lastPriceReduction: listingDetails.lastPriceReduction,
    listingAgent: listingAgent
  }
