...
async function updateInventoryWithSales(userId, completedSales = []) {
    ...
    let shippingTransactions = []; //May go away
    const shippingTransactionsRequest = await getShippingTransactions(ebayOAuthToken); //May go away
    shippingTransactions = shippingTransactionsRequest.data ? shippingTransactionsRequest.data.transactions : []; //May go away
...
async function getSaleInfo(sale, shippingTransactions) {
    const { Item: { ItemID },
        PaidTime, FinalValueFee, ShippingDetails, ConvertedTransactionPrice, Buyer, purchasePrice, inventoryItemId, ExtendedOrderID, OrderLineItemID } = sale;
...
async function updateAllZeroShippingCost(userId) {
    ...
    // console.log(zeroCostList)
    let shippingTransactions = [];
    try {
        const shippingTransactionsRequest = await getShippingTransactions(ebayOAuthToken);
        shippingTransactions = shippingTransactionsRequest.data ? shippingTransactionsRequest.data.transactions : [];
