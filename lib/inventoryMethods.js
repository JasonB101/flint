...
async function verifyCorrectPricesInInventoryItems(inventoryItems, ebayListings, averageShippingCost) {
    ...
    const updates = await Promise.all(
        ebayListings.map(listing => {
            const { BuyItNowPrice, ItemID, OrderLineItemID } = listing;
            const found = inventoryItems.find(item => (item.ebayId === ItemID && (+item.listedPrice !== +BuyItNowPrice || item.OrderId !== OrderLineItemID)));
            if (found) {
                console.log("Verify: ",found.title);
