...
async function updateInventoryWithSales(
  ...
  const itemsUpdated = await Promise.all(
    salesToUpdateWith.map(async (sale) => {
      ...
      const profit = +(priceSold - purchasePrice - ebayFees - shipping - additionalCost).toFixed(
        2
      )
      // console.log(profit)
      const updates = {
...
function getSalesToUpdateWith(userId, completedSales = [], inventoryItemsMap) {
  ...
  const salesToUpdateWith = completedSales.map((sale) => {
    const { lineItems, cancelStatus } = sale
    ...
    try {
      ...
      if ((cancelled && inventoryItem?.listed) || inventoryItem?.shipped) {
        inventoryItem = null
      }
      if (inventoryItem) {
        ...
        sale.additionalCost = additionalCosts.reduce((acc, cost) => acc + cost.amount, 0)
