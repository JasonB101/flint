...
async function getInventoryItems(userId, filterListed = null) {
  const query = { userId };
  if (filterListed !== null) {
...
async function verifyCorrectPricesInInventoryItems(
  ...
  const inventoryMap = new Map(
    inventoryItems.map(item => [item.ebayId, item])
  );
  
  const updates = await Promise.all(
    ebayListings
      .filter(listing => {
        const { BuyItNowPrice, ItemID, Title, WatchCount = 0 } = listing;
        const inventoryItem = inventoryMap.get(ItemID);
        return inventoryItem && (
          +inventoryItem.listedPrice !== +BuyItNowPrice ||
          inventoryItem.title !== Title ||
          inventoryItem.watchers !== +WatchCount
        );
      })
      .map(listing => updateItemWithEbayListing(
        inventoryMap.get(listing.ItemID)._id,
        listing,
        averageShippingCost,
        ebayFeePercent
      ))
...
function figureExpectedProfit(
  ...
  console.log(listedPrice, purchasePrice, averageShippingCost, ebayFeePercent) 
  const taxOnListedPrice = +listedPrice * +estimatedTaxRate
  const ebayFee = +(+listedPrice + +taxOnListedPrice) * +ebayFeePercent
  const expectedProfit =
    +listedPrice - +ebayFee - +averageShippingCost - +purchasePrice
