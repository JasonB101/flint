      ...
      .filter((listing) => {
        ...
        return (
          ...
          (+inventoryItem.listedPrice !== +BuyItNowPrice ||
            ...
            inventoryItem.watchers !== +WatchCount || inventoryItem.ebayId !== ItemID)
        )
      })
      .map((listing) =>
        updateItemWithEbayListing(
          inventoryMap.get(listing.SKU)._id,
          listing,
          averageShippingCost,
          ebayFeePercent
        )
...
async function updateItemWithEbayListing(
  ...
  const { BuyItNowPrice, OrderLineItemID, Title, WatchCount, ItemID: ebayId } = ebayListing
