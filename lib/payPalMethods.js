    ...
    try {
        ...
        if (payPalResponse.data) {
            ...
            if (data.access_token) {
                ...
                const updatedUser = await updateUserWithToken(req.user._id, token)
                return res.send({ success: true, user: updatedUser })
            } else {
                throw "Missin Acces Token"
            }
        } else {
...
async function getPayPalTransactions(userId) {
    ...
    const numberOfTransactionDays = 3;
    ...
    const transactions = await payPalGetRequest(url, config)
    return transactions;
}

async function payPalGetRequest(url, config) {
    try {
