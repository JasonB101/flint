...
async function getCompletedSales(ebayAuthToken) {
    ...
    try {
        ...
        if (data) {
            ...
            let transactions = response ? [ response.TransactionArray.Transaction ] : []
            return transactions //This wrapped array is temporary ^^^^, there is only 1 transaction currently, when it hits 2 this will be an array already
        }
    } catch (e) {
        console.log(e)
...
async function getShippingTransactions(ebayOAuthToken) {
    ...
    try {
        const transactions = await axios.get("https://apiz.ebay.com/sell/finances/v1/transaction?filter=transactionType:{SHIPPING_LABEL}"
            , config)
