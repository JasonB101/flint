...
const createEbayDescriptionTemplate = require("../lib/createEbayDescriptionTemplate")
...
async function getCompletedSales(ebayAuthToken) {
    ...
    try {
        ...
        if (data) {
            ...
            let transactions = Array.isArray(array) ? array : [array] //If there is only one transaction, it returns as an object instead of array
            return transactions 
        }
    } catch (e) {
        console.log(e)
...
async function createListing(ebayAuthToken, listingDetails) {
    ...
    const quantity = 1;
    const { title, brand, sku, listedPrice, conditionId, conditionDescription, acceptOfferHigh, declineOfferLow, description,
        categoryId, partNo, shippingService } = listingDetails;
    const config = {
        headers: {
            'Content-Type': 'text/xml',
    ...
    <AddFixedPriceItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
      <RequesterCredentials>
        ...
        <Item>
            ...
            <Description><![CDATA[${createEbayDescriptionTemplate(title, partNo, description, shippingService)}]]></Description>
...
async function getShippingTransactions(ebayOAuthToken) {
    ...
    try {
        const transactions = await axios.get(`https://apiz.ebay.com/sell/finances/v1/transaction?filter=transactionType:{SHIPPING_LABEL}&filter=transactionDate:[${new Date(new Date() - 30 * 86400000).toISOString()}]`
            , config)
            
            let shippingTransactions = transactions.data ? transactions.data.transactions : []; //May go away
