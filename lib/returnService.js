...
/**
 * Create notification when a return is auto-processed as re-listed
 * @param {Object} returnRecord - The return record
 * @param {Object} inventoryItem - The inventory item 
 * @param {String} userId - User ID
 */
async function createAutoRelistNotification(returnRecord, inventoryItem, userId) {
  try {
    // Check user's notification settings before creating notification
    const user = await User.findById(userId).select('notificationSettings')
    ...
    // Only create notification if user has automatic returns enabled
    if (!notificationSettings.automaticReturns) {
      console.log(`Auto-relist notification skipped for User ${userId} - disabled in settings`)
      return
    }

    // Check if notification already exists for this return processing
    const existingNotification = await Notification.findOne({
      userId: userId,
      type: 'autoRelistProcessed',
      'data.ebayReturnId': returnRecord.ebayReturnId,
      isDeleted: { $ne: true } // Don't count deleted notifications
    })
    ...
    if (!existingNotification) {
      ...
      const notification = new Notification({
        ...
        data: {
          ...
          message: `Return auto-processed for ${inventoryItem.title} (SKU: ${inventoryItem.sku}) - Item restored to active listing`
        },
        type: 'autoRelistProcessed',
        isViewed: false,
      })
      ...
      console.log(`🔔 Auto-relist notification created for User ${userId} - Return ${returnRecord.ebayReturnId}`)
    } else {
      console.log(`Auto-relist notification already exists for return ${returnRecord.ebayReturnId}`)
    }
  } catch (error) {
    console.error(`Error creating auto-relist notification: ${error.message}`)
  }
}
      ...
      if (shouldAutoProcess && !dryRun) {
        try {
          ...
          if (recommendation.action === 'WASTE') {
            ...
            inventoryUpdates.dateWasted = new Date().toLocaleDateString() // CRITICAL: Mark when item was wasted
            ...
            // PRESERVE buyer information for waste tracking - DO NOT set to null
            // inventoryUpdates.buyer = null  // ❌ REMOVED - Keep buyer info
...
/**
 * Server-side version of itemReListed processor
 * @param {Object} newValues - Return processing values
 * @returns {Object} Update object for MongoDB
 */
const createRelistUpdates = (newValues) => {
  const { dateListed, additionalCosts, expectedProfit, ebayId, itemId, returnDate, orderId } = newValues
  
  return {
    // Unset (clear) sale data
    $unset: {
      priceSold: "",
      dateSold: "",
      ebayFees: "",
    ...
    $set: {
      ...
      status: "active",
      listingAgent: "member",
      automaticReturn: true, // Mark as automatic return processing
      returnDate: returnDate || new Date().toLocaleDateString(),
      lastReturnedOrder: orderId,
      returnCount: newValues.returnCount || 1
    }
  }
}

/**
 * SIMPLE forward-going return processing - uses existing itemReListed processor
 * @param {String} userId - User ID
 * @param {Array} activeListings - Array of active eBay listings
 * @param {Object} options - Processing options
 * @returns {Object} Processing results
 */
const processRelistedReturns = async (userId, activeListings = [], options = {}) => {
  const { dryRun = false } = options

  try {
    console.log(`🔄 Processing re-listed returns for user ${userId}`)
    ...
    for (const returnItem of unprocessedReturns) {
      // Simple check: Is this item currently listed on eBay?
      if (returnItem.sku && returnItem.sku.trim()) {
        const isCurrentlyListed = activeListings.some(listing => 
          (listing.SKU && listing.SKU === returnItem.sku) || 
          (listing.sku && listing.sku === returnItem.sku)
        )

        if (isCurrentlyListed) {
          console.log(`✅ Found re-listed return: ${returnItem.sku} (${returnItem.ebayReturnId})`)
          ...
          if (!dryRun) {
            try {
              ...
              // Prepare return costs for additional costs
              const additionalCosts = [...(inventoryItem.additionalCosts || [])]
              
              if (returnItem.returnShippingCost > 0) {
                additionalCosts.push({
                  title: 'returnShippingCost',
                  amount: returnItem.returnShippingCost,
                  date: new Date()
                })
              }
              ...
              // Calculate expected profit (basic calculation - could be enhanced)
              const expectedProfit = inventoryItem.expectedProfit || 0

              // Prepare values for existing itemReListed processor
              const relistValues = {
                dateListed: new Date().toLocaleDateString(),
                additionalCosts: additionalCosts,
                expectedProfit: expectedProfit,
                ebayId: inventoryItem.ebayId,
                itemId: inventoryItem._id,
                returnDate: returnItem.creationDate ? new Date(returnItem.creationDate).toLocaleDateString() : new Date().toLocaleDateString(),
                orderId: returnItem.orderId,
                returnCount: (inventoryItem.returnCount || 0) + 1
              }

              // Use existing itemReListed processor logic
              const updates = createRelistUpdates(relistValues)

              // Update inventory item using existing processor logic
              await InventoryItem.findByIdAndUpdate(inventoryItem._id, updates)

              // Create notification for auto-processing
              await createAutoRelistNotification(returnItem, inventoryItem, userId)
              ...
              console.log(`   ✅ Processed as re-listed item using existing processor`)
              results.processed++
              results.relistProcessed++

            } catch (error) {
              console.error(`   ❌ Error processing return ${returnItem.ebayReturnId}:`, error.message)
              results.skipped++
            }
          } else {
            console.log(`   🔍 Would process as re-listed (dry run)`)
            results.relistProcessed++
          }
        } else {
          results.skipped++
        ...
        console.log(`⚠️ No SKU for return ${returnItem.ebayReturnId} - cannot check listing status`)
        results.skipped++
      }
    }

    console.log(`\n📊 Simple Processing Results:`)
    console.log(`   Re-listed items processed: ${results.relistProcessed}`)
    console.log(`   Skipped: ${results.skipped}`)

    return results

  } catch (error) {
    console.error('Error in simple return processing:', error)
    throw error
  }
}
...
module.exports = {
  ...
  createReturnDeliveredNotification,
  createAutoRelistNotification
} 